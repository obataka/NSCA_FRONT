...
class Vmoshikomi_jokyo
{
	...
	FROM (
		...
		SELECT 
			...
			issue.settleno,
			issue.kessai_kekka,
			ifnull(issue.status,'')    AS status,
			issue.koshin_user_id,
			ifnull(issue.pay_type_specify,'')    AS pay_type_specify,
			issue.koshin_nichiji,
			now()			AS syutoku_nichiji,
			detail.etc_id,
		...
		SELECT 
			...
			vmoshikomi_jokyo.id,
			ifnull(nonyubi,'')	AS nonyubi,
			settleno,
			kessai_kekka,
			ifnull(status,'')	AS status,
			koshin_user_id,
			ifnull(pay_type_specify,'')	AS pay_type_specify,
			koshin_nichiji,
			now()			AS syutoku_nichiji,

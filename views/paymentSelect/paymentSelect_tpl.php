...
<html lang="ja">
	<head>
		...
		<script type="text/javascript" src="../../viewIncludeFiles/js/paymentSelect.js"></script>
	</head>
	<body>
		<?php
			$headerHtmlVisitor = '
			...
			if ($memberType == $payCons->PROC_TYPE_VISITOR) {
				// 非会員
				$contentClassName = 'wrap_visitor';
				echo $headerHtmlVisitor;
			} else {
				// 会員
				$contentClassName = 'wrap_member';
				echo $headerHtmlMember;
			}
		?>
		<div class="wrap mh_c <?php echo $contentClassName ?>">
			<h1>決済方法選択</h1>
			<div class="content_wrap">
				<p class="seikyu">請求内容</p>
				<form action="?" method="post" autocomplete="off" id="confirmForm" enctype="multipart/form-data">
					<input type="hidden" name="tranScreen" id="tranScreen" value="<?php echo $tranScreen; ?>">
					<div class="bg_gray">
					...
					<div class="shiharai_naiyo">
						<table>
							...
							<tr>
								<td id="pay_customer_name"><?php echo $shimeiSei; ?>　<?php echo $shimeiMei; ?>　様</td>
								<td id="pay_title"><?php echo $payName; ?></td>
								<td id="pay_money"><?php echo $payment; ?>円</td>
							</tr>
						</table>
					</div>
